...
def mfitpoly(x, t, model_order):
    """
    This is for calculation of w based on x and t
    ...
    #plotX[:, k] = numpy.power(plotx, k)
    w = numpy.dot(numpy.linalg.inv(numpy.dot(numpy.transpose(X),X)),numpy.dot(numpy.transpose(X),t))  # Calculate w vector (as an numpy.array)
